 ...
 * snapshots may re-render this component, but only the button deals a new
 * order. Hero keys it by pool and spelling so an edited setup gets a new list.
 */
export function NoteList({ pool, spelling, random }: NoteListProps) {
  ...
  return (
    <div className="note-list-block">
      ...
      <button
        ...
        onClick={() => setNotes(dealList(pool, spelling, random))}
      >
        <FontAwesomeIcon icon={faShuffle} aria-hidden="true" /> Regenerate list
      </button>
    </div>
  )
}
